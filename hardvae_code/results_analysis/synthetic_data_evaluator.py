...
-- Modifications:
- Modify the topological evaluation to use the IsUMap dimensionality reduction technique instead of PCA to better preserve topological features.
- Implement sensitivity analysis for the topological evaluation by varying the size of the embedded space and assessing the stability of the results.
- Ensure that the topological evaluation uses the entire dataset without subsampling to capture the full topological structure.
- How to select the values for the IsUMap embedding space size?
- Our datsaets feature space range is [5, 31] so the values can be {2, 3, 4, 5, 6, 7, 8, 9, 10} for the embedded space size. We can then analyze the stability of the TDA results across these different embedding sizes to ensure robustness.
- Thus the results should be saved separately for each dataset and for each view of evaluation
- to be able to make the sensitivity analysis for datasets with different feature space sizes we can save the results in a way that allows us to compare the TDA results across different embedding sizes for each dataset. For example, we can create a dictionary structure where the keys are the dataset names and the values are another dictionary containing the TDA results for each embedding size. This way, we can easily analyze how the TDA results change with different embedding sizes for each dataset.

**Updates:**
    *   Update the the hardness_metrics in evaluate hardness function
    *   Remove `Usefulness` from the hardness metrics
...
import sys
# On cherche le dossier 'isumap/src' par rapport à l'emplacement de ce script
# On remonte d'un niveau (parent) puis on descend dans isumap/src
current_dir = Path(__file__).resolve().parent.parent
isumap_src_path = current_dir / "isumap" / "src"

if isumap_src_path.exists():
    sys.path.append(str(isumap_src_path))
    from isumap import isumap
    print("IsUMap chargé avec succès !")
else:
    print(f"Erreur : Le dossier {isumap_src_path} est introuvable.")
...
class SyntheticDataEvaluator:
    ...
    def hardness_evaluation(self, X_real, y_real, X_synth, y_synth, 
        ...
        if hardness_metrics is None:
            hardness_metrics = ['kDN', 'DS', 'DCP', 'TD_P',
                            'TD_U', 'CL', 'CLD', 'MV', 
                            'CB', 'N1', 'N2', 'LSC', 
                            'LSR', 'Harmfulness',
                            'F1', 'F2', 'F3', 'F4']
    ...
    def topological_evaluation(self, X_real, y_real, X_synth, y_synth, save_path=None, dataset_name="dataset", minority_class=None):
        ...
        USE IsUMap instead of PCA for dimensionality reduction to preserve topological features better.
        Perform sensitivity analysis to the size of the embedded space to ensure the robustness of the evaluation.
        Use the whole dataset for TDA analysis without subsampling.
        ...
        for d_val in dimensions_to_test:
            print(f"\n--- Evaluating with IsUMap (d={d_val}) ---")
            results = {'runs':[]}

            try:                
                # CHANGE HERE FOR IsUMap
                _, X_real_embedded, _ = isumap(data = X_real_min, d=d_val,k=15, verbose=True)
                _, X_synth_embedded, _ = isumap(data = X_synth, d=d_val,k=15, verbose=True)
                
                # For k times sample equal sets from X_real_embedded and X_synth_embedded, where the size is random between in the range((half the size of the smaller set), (the size of the smaller set))
                b = min(X_synth_embedded.shape[0] , X_real_embedded.shape[0])
                a = max(10, int(0.5*b))
                for k in range(0,3):
                    ...
                    idx_real = np.random.choice(X_real_embedded.shape[0], r_sample_size, replace=False)
                    idx_synth = np.random.choice(X_synth_embedded.shape[0], r_sample_size, replace=False)

                    
                    # sample r_sample_size from X_real_pca and X_synth_pca
                    X_real_sample = X_real_embedded[idx_real]
                    X_synth_sample = X_synth_embedded[idx_synth]
                    ...
                    for homology_dim in [0,1]:
                        if homology_dim < len(dgm_real) and homology_dim < len(dgm_synth):
                            ...
                            run_results[f'bottleneck_H{homology_dim}'] = float(bottleneck(dgm_r, dgm_s))
                            run_results[f'wasserstein_H{homology_dim}'] = float(wasserstein(dgm_r, dgm_s))
                ...
                if save_path:
                    ...
                    plot_diagrams(dgm_real, ax=axes[0], title=f'Real Data - {dataset_name} (d={d_val})')
                    plot_diagrams(dgm_synth, ax=axes[1], title=f'Synthetic Data - {dataset_name} (d={d_val})')
                    plt.tight_layout()
                    plt.savefig(os.path.join(save_path, f'persistence_diagrams_{dataset_name}_d{d_val}.png'),
                                dpi=100, bbox_inches='tight')
                    plt.close()
                ...
                print(f"Warning: Topological evaluation failed for d={d_val}: {e}")
                results['error'] = str(e)

            all_results[f"d_{d_val}"] = results
...
if __name__ == '__main__':
    ...
    X_real_dummy = np.random.rand(160, 10)
    y_real_dummy = np.array([0]*120 + [1]*40)
    X_synth_dummy = np.random.rand(80, 10)
    y_synth_dummy = np.array([1]*80)

    evaluator = SyntheticDataEvaluator()
    # tf = evaluator.topological_evaluation()
    results = evaluator.topological_evaluation(X_real_dummy, y_real_dummy, X_synth_dummy, y_synth_dummy, save_path='./')
