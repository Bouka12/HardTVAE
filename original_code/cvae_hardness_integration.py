...
def main():
    ...
    for dataset_name, hardness_metric, seed, weighting_strategy in combinations: #itertools.product(datasets, hardness_metrics, seeds, weighting_strategies):
        ...
        # try:
        #     hardness_scores = hardness_calc.calculate_hardness_scores(X_train, y_train, [hardness_metric])
        #     # print(f"Hardness scores shape: {hardness_scores.shape}")
        #     # print("Hardness summary:")
        #     # print(hardness_calc.get_summary_statistics(hardness_scores))
        # except Exception as e:
        #     print(f"Error calculating hardness: {e}")
        #     return
        ...
        # the majority class label is the one with the most samples check Counter to see the label and its count
        ...
        # Fidelity evaluation:
        # X_synth = synthetic_samples.cpu().numpy()
        # y_synth = np.full((synthetic_samples.shape[0], 1)
        ...
        # if os.path.exists(csv_path):
        #     pd.DataFrame([ks_record]).to_csv(ks_results_path, mode='a', header=False , index=False)
        # else:
        #     pd.DataFrame([ks_record]).to_csv(ks_results_path, index=False)
        print("="*50)
        print("="*50)
        ...
        # print(f"Generated {synthetic_samp.shape[0]} synthetic minority samples")
        # print(f"Synthetic sample statistics:")
        # print(f"Mean: {synthetic_samples.mean(dim=0)}")
        # print(f"Std: {synthetic_samples.std(dim=0)}")
        
        # print(f"Combined dataset shape: {combined_data.shape}")
        # print(f"Class distribution after synthesis: {np.bincount(combined_data['label'].astype(int))}\\n")
