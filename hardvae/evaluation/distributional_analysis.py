"""This script processes KS statistic results to generate heatmaps and summary tables.
"""
...
df.columns = ['dataset', 'hardness_metric', 'random_seed', 'weighting_strategy', 'ks_mean', 'ks_std']
...
output_dir = Path(r'C:\Users\BOUKA\Downloads\PART2-Hardness-CSVAE\Hard-CSVAE\Fidelity_Viz\Similarity_plots')
output_dir.mkdir(exist_ok=True)

# Save filtered Distributional fidelity results to CSV
save_path = output_dir / "filtered_Distributional_results.csv"
...
plt.axhline(
    ...
    linestyle = '--',
    linewidth=2,
    label=fr"$\textit{{Baseline Mean}} = {baseline_mean:.3f}$"
)

plt.xlabel("", fontsize=14)
plt.ylabel(r"\textit{Mean of KS statistic}", fontsize=10)
# plt.title("Weighting Strategies vs Baseline", fontsize=16)
plt.legend()
plt.tight_layout()
pathC = output_dir / f"distributional_boxplot_strategies_vs_baseline_mean_{VIZ_METRIC}.png"
...
plt.ylabel(r"\textit{Mean of KS statistic}", fontsize=14)
# plt.title(r"\textit{Distributional Fidelity per Hardness Metric (Averaged over Strategies)}", fontsize=16)
# plt.xticks(rotation=45)
plt.legend()
plt.tight_layout()

box_path = output_dir / f"distributional_boxplot_per_metric_avg_{VIZ_METRIC}.png"
...
##################################
## Heatmap data and visualization generation
##################################
## Uncomment if needed
...
#     pivot_table_path_csv = output_dir / f'{dataset_name}_Similarity_ks_mean_table.csv'
#     pivot_table_path_tex = output_dir / f'{dataset_name}_Similarity_ks_mean_table.tex'
...
#     # KS Mean heatmap
#     sns.heatmap(pivot_mean, annot=True, fmt='.3f', cmap='YlGnBu', #cmap='viridis', 
...
#     # Save the plot
#     output_path = output_dir / f'{dataset}_SimilarityKS_heatmap.png'
...
# sns.heatmap(summary_pivot_mean, annot=True, fmt='.3f', cmap='YlGnBu', vmin=vmin,
#                 vmax=vmax, ax=ax,
#                 annot_kws={'size': 14},  # Increase font size for annotations
#                 cbar_kws={'label': 'KS Mean'})
...
# # Save summary plot
# summary_path = output_dir / 'summary_all_datasets_corrected_heatmap.png'
...
# ax1.set_title('Distribution of KS Statistics by Weighting Strategy')
...
# plt.tight_layout()
# plt.savefig(output_dir / 'SimilarityKS_weighting_strategy_boxplots.png', dpi=300, bbox_inches='tight')
# plt.close()